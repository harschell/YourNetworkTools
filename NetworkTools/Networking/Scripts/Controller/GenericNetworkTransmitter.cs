	...
	public class GenericNetworkTransmitter : MonoBehaviour
	{
		...
		public static GenericNetworkTransmitter Instance
		{
			get
			{
				if (!instance)
				{
					instance = GameObject.FindObjectOfType(typeof(GenericNetworkTransmitter)) as GenericNetworkTransmitter;
				}
				return instance;
			}
		}
		...
		private void NetworkListener_ConnectionReceived(StreamSocketListener sender, StreamSocketListenerConnectionReceivedEventArgs args)
		{
			...
			{
				Debug.Log("No data to send but we've been connected to.  This is unexpected.");
			}
		}
